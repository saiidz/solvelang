...
async function replaceExact(filePath, from, to) {
  ...
  assert.notEqual(first, -1, `${path.basename(filePath)} is missing the expected published-version runtime marker`);
  assert.equal(text.indexOf(from, first + from.length), -1, `${path.basename(filePath)} has an ambiguous duplicate runtime marker`);
...
const publishedManifest = await readJson(path.join(packageRoot, "package.json"));
const publishedLock = await readJson(path.join(packageRoot, "package-lock.json"));
...
assert.equal(publishedManifest.version, candidate.publishedVersion, "repository package metadata must still describe the published line before finalization");
assert.equal(publishedLock.version, candidate.publishedVersion, "repository lockfile must still describe the published line before finalization");
assert.equal(publishedLock.packages?.[""]?.version, candidate.publishedVersion, "repository lockfile root package must still describe the published line before finalization");
...
try {
  ...
  assert.doesNotMatch(installedIndex, new RegExp(`version: ["']${candidate.publishedVersion.replaceAll(".", "\\.")}["']`));
  ...
  for (const name of [
    ...
    assert.equal(toolNames.has(name), true, `staged ${candidate.candidateVersion} package is missing ${name}`);
  }

  console.log(
    `SolveLang MCP ${candidate.candidateVersion} staged finalization PASS: clean tarball install and Solve Context startup verified; repository/public line remains ${candidate.publishedVersion}; publication remains unauthorized.`,
  );
} finally {
  try {
    await client?.close();
