...
test("rejects baseline records that claim a Solve Context pack", () => {
  const value = record("baseline");
  value.context.packId = packId;
  value.context.selectedBytes = 123;
  assert.throws(() => validateContextAgentEvalRecord(value), /Baseline records cannot claim a Solve Context pack/);
});

test("reports provider token, latency, evidence, and cache-hot measurements without authorizing a claim", () => {
