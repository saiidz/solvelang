...
- The browser/WASM safety train includes a pure host-incapable `solvec-core`, a deny-all wrapper, shared conformance, reproducible audited packages and real-browser loader qualification. The repository's `/run/` now consumes a reviewed static artifact pin with visible fail-closed loading. This does not establish a live deployment or managed execution authority.
...
- pure host-incapable Rust evaluator core and a deny-all WASM wrapper with shared semantic-conformance/resource-limit coverage, wired to `/run/` through a reviewed static artifact pin; deployment is a separate gate.
...
4. preserve the ADR 0002 artifact and browser parity gates on every runtime change; qualify and review new static pins before updating `/run/`, with live deployment independently verified;
