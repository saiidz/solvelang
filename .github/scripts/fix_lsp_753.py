...
s = s.replace(
    ...
    'ast::{Expr, ExprKind, ExportedDeclaration, SourceLocation, Stmt},',
    1,
)
start = s.index('fn normalize_uri_path')
end = s.index('fn binding_declaration_indexes')
replacement = r'''fn resolve_import_uri(importer: &str, import_path: &str) -> Option<String> {
...
test_end = s.index('    #[test]\n    fn repeated_cross_file_responses_are_deterministic()', test_start)
tests = r'''    #[test]
    ...
    fn object_literal_iterable_keeps_loop_shadow_active_in_body() {
        ...
        let source = "import \"item.solve\" as item\nfor item in [{ value: 1 }] { print(item.value) }\n";
