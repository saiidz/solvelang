...
export default function RunPage() {
  ...
  const [session] = useState(() => createCanonicalPreviewSession(async () => {
    const { loadAuditedWasm } = await import("../../../../solvec-wasm/browser/loader.mjs");
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-5 py-8 text-white sm:px-6 sm:py-12">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/10 p-5 text-sm leading-6 text-amber-100"><h2 className="font-semibold text-white">Capability boundary</h2><p className="mt-2">This browser preview uses a pinned safe-core build. Native <code>solvec</code> remains canonical for the full language and host capabilities. General managed production workflow execution is not live.</p></section>
      </div>
    </main>
  );
}
