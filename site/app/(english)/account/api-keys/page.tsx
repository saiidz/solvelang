...
const plans = [
  { key: "developer", name: "Developer", requests: "1,000 requests/month", keys: "2 active keys" },
  { key: "pro", name: "Pro", requests: "10,000 requests/month", keys: "3 active keys" },
  { key: "business", name: "Business", requests: "50,000 requests/month", keys: "5 active keys" },
] as const;
  ...
  async function startCheckout(plan: "developer" | "pro" | "business") {
    ...
    try {
      const result = await customerApi<{ url: string }>(API_BASE, "/customer/subscriptions/checkout", {
        method: "POST",
        csrfToken: dashboard.csrfToken,
        body: JSON.stringify({ plan, requestId: newRequestId() }),
      });
      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col gap-5 border-b border-white/10 pb-8 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-bold">Keys, subscription, and usage</h1>
        ...
        <section className="mt-8 grid gap-5 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6"><p className="text-sm text-slate-400">Current usage</p><p className="mt-2 text-2xl font-bold">{dashboard.usage.used ?? 0} / {dashboard.usage.limit ?? "—"}</p><p className="mt-1 text-sm text-slate-300">Period {dashboard.usage.period}</p></div>
        ...
        {!dashboard.subscription.plan ? (
          <section className="mt-8 rounded-3xl border border-cyan-300/30 bg-cyan-300/5 p-6">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              {plans.map((plan) => <div key={plan.key} className="rounded-2xl border border-white/10 bg-slate-950/50 p-5"><h3 className="text-xl font-bold">{plan.name}</h3><p className="mt-3 text-sm text-slate-300">{plan.requests}</p><p className="mt-1 text-sm text-slate-300">{plan.keys}</p><button disabled={busy} onClick={() => startCheckout(plan.key)} className="mt-5 w-full rounded-xl bg-cyan-300 px-4 py-2 font-bold text-slate-950 disabled:opacity-60">Start checkout</button></div>)}
            </div>
          </section>
        ) : null}
            ...
            <div className="mt-5 space-y-3">
              {dashboard.keys.length === 0 ? <p className="rounded-xl border border-dashed border-white/15 p-6 text-slate-400">No API keys yet.</p> : dashboard.keys.map((key) => (
                <article key={key.keyId} className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-black/15 p-5 md:flex-row md:items-center md:justify-between">
                  <div><h3 className="font-bold">{key.name}</h3><p className="mt-1 font-mono text-sm text-slate-300">{key.prefix}…{key.lastFour}</p><p className="mt-2 text-xs text-slate-500">Created {readableDate(key.createdAt)} · Last used {readableDate(key.lastUsedAt)}</p></div>
