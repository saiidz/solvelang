...
export default function ScenarioLab({ workflow, selectedScenarioId, onSelectScenario, onUpdateScenarios, onRun, onOpenComparison }: {
  ...
  const parseInputs = (value: string) => Object.fromEntries(value.split("\n").map((line) => { const separator = line.indexOf("="); return separator < 0 ? [line.trim(), ""] : [line.slice(0, separator).trim(), line.slice(separator + 1).trim()]; }).filter(([key]) => key && !["__proto__", "prototype", "constructor"].includes(key)));
  ...
  const remove = () => { if (!scenario || !window.confirm(`Delete scenario ${scenario.name}?`)) return; const remaining = workflow.scenarios.filter((item) => item.id !== scenario.id); onUpdateScenarios(remaining); onSelectScenario(remaining[0]?.id ?? ""); };
    ...
    <section className={styles.viewPanel} aria-labelledby="scenario-title">
      ...
      <div className={styles.scenarioLayout}>
        ...
        {scenario ? <div className={styles.scenarioEditor}>
          ...
          <label className={styles.field}><span>Input variables (one key=value per line)</span><textarea rows={3} value={Object.entries(scenario.inputVariables).map(([key, value]) => `${key}=${value}`).join("\n")} onChange={(event) => update({ inputVariables: parseInputs(event.target.value) })} /></label>
