...
export default function StudioApp() {
  ...
  return (
    <div className={styles.studio}>
      ...
      <header className={styles.topbar}><Link href="/" aria-label="SolveLang home"><Image src="/solvelang-mark-mono.svg" alt="" width={32} height={32} priority /><span className={styles.brandText}>Solve<em>Lang</em></span></Link><div className={styles.projectIdentity}><input aria-label="Project name" value={workflow.name} onChange={(event) => mutate((current) => { current.name = event.target.value; return current; })} /><span>{workflow.version} · {saveStatus}</span></div><div className={styles.topActions}><span className={styles.localBadge}>Local-first</span><Link href="/run/">Script preview</Link><Link href="/">Exit Studio</Link></div></header>
