...
} from "./core/angularTargetConfigPresentation";

type RepositoryAuditAngularTargetConfigPanelProps = {
  presentation: RepositoryAngularTargetConfigPresentation;
  className?: string;
};

const targetStateClasses: Record<RepositoryAngularTargetConfigPresentationRow["targetState"], string> = {
...
function targetStateLabel(state: RepositoryAngularTargetConfigPresentationRow["targetState"]): string {
  if (state === "outside-bounded-scan") return "outside bounded scan";
  return state;
}

export function RepositoryAuditAngularTargetConfigPanel({
  presentation,
  className = "",
}: RepositoryAuditAngularTargetConfigPanelProps) {
  ...
  return (
    <section className={`rounded-[2rem] border border-violet-200 bg-violet-50 p-6 shadow-sm sm:p-8 ${className}`.trim()}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-violet-800">Angular target configuration</p>
          <h2 className="mt-2 text-2xl font-semibold text-slate-950">Explicit target tsConfig relationships</h2>
        </div>
      ...
      <p className="mt-3 max-w-4xl leading-7 text-slate-700">
        SolveLang maps only explicit repository-local Angular target <code>options.tsConfig</code> references from bounded configuration evidence. Dynamic, absolute, globbed, malformed, or otherwise ambiguous references are skipped instead of guessed.
      </p>
      ...
      <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-slate-700">
        ...
        {presentation.summary.rowsHidden > 0 ? (
          <span className="rounded-full bg-violet-100 px-3 py-1.5 text-violet-900">{presentation.summary.rowsHidden} hidden by row limit</span>
        ) : null}
      </div>
        ...
        {presentation.rows.length ? presentation.rows.map((row) => (
          <article key={row.evidenceId} className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-bold uppercase text-violet-900">
                Angular
              </span>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                {row.project} · {row.target}
              </span>
          ...
          <div className="rounded-2xl border border-dashed border-violet-200 bg-white p-8 text-center text-sm text-slate-500">
            No explicit bounded Angular target tsConfig relationships were identified.
          </div>
        )}
      </div>
