...
test("maps package entrypoints without treating out-of-scope files as scanned", async () => {
  ...
  assert.deepEqual(analysis.references.map((reference) => [reference.evidence.field, reference.targetPath, reference.targetState]), [
...
test("records only explicit repository-local GitHub Action references", async () => {
  const workflowText = ["jobs:", "  verify:", "    steps:", "      - uses: actions/checkout@v4", "      - uses: ./.github/actions/setup", "      - uses: './.github/actions/missing'"].join("\n");
  const input = snapshot([
    { path: ".github/workflows/verify.yml", text: workflowText },
    { path: ".github/actions/setup/action.yml", text: "name: setup" },
  ]);
  const analysis = await createRepositoryConfigurationReferenceAnalysis(input, await graph([".github/workflows/verify.yml", ".github/actions/setup/action.yml"]));
  assert.deepEqual(analysis.references.map((reference) => [reference.rawReference, reference.targetPath, reference.targetState]), [
