...
function resolveRelativePath(baseDirectory: string, rawReference: string): string | undefined {
  ...
  if (rawReference.startsWith("/") || /^[A-Za-z]:/.test(rawReference) || /^[A-Za-z][A-Za-z0-9+.-]*:/.test(rawReference)) {
    return undefined;
  }
...
): RepositoryConfigurationTargetState {
...
function packageReferences(
  ...
  try {
    ...
    return { references: [], invalidJson: true };
  }
  ...
  if (typeof bin === "string" && bin.length > 0) {
    values.push({ field: "bin", value: bin });
...
export async function createRepositoryConfigurationReferenceAnalysis(
  ...
  const maxConfigTextBytes = boundedInteger(
    ...
    1,
    10 * 1024 * 1024,
    "Repository configuration maxConfigTextBytes",
  );

  // Building the query index validates canonical graph integrity before configuration
  // evidence is allowed to depend on its bounded accepted-file surface.
  await createSolveGraphQueryIndex(graph);
  if (!sameSource(snapshot, graph)) {
    throw new Error("Repository configuration evidence source does not match the bounded Solve Graph source.");
  }
  ...
  allReferences.sort((left, right) =>
    ...
    || compareText(left.rawReference, right.rawReference)
