...
export type RepositoryWorkflowPathReferenceKind = "working-directory" | "cache-dependency-path";
export type RepositoryWorkflowPathTargetState = "present" | "outside-bounded-scan" | "missing";
...
export type RepositoryWorkflowPathEvidenceAnalysis = {
  ...
  skipped: { missingText: number; oversizedText: number; dynamicReferences: number; multilineReferences: number };
  execution: {
    networkAccess: false;
    writeAccess: false;
...
export type RepositoryWorkflowPathEvidenceOptions = { maxReferences?: number; maxWorkflowTextBytes?: number };
...
function sameSource(snapshot: RepositorySnapshot, graph: SolveGraphDocument): boolean {
  return snapshot.source.displayName === graph.source.displayName && snapshot.source.revision === graph.source.revision && snapshot.source.fingerprint === graph.source.fingerprint;
}
...
function textBytes(text: string): number { return new TextEncoder().encode(text).byteLength; }
...
function isDynamicOrPatternReference(value: string): boolean {
  return value.length === 0 || value.includes("${{") || value.includes("$(") || /[*?{}[\]]/.test(value) || value.includes("\0") || value.includes("\\") || value.startsWith("/") || /^[A-Za-z]:/.test(value) || /^[A-Za-z][A-Za-z0-9+.-]*:/.test(value);
}
function normalizeWorkflowReference(value: string): string | undefined {
  ...
  try { return normalizeRepositoryPath(stripped); } catch { return undefined; }
}
function makeReferenceId(workflowPath: string, kind: RepositoryWorkflowPathReferenceKind, targetPath: string, line: number): string {
  return `workflow-path:${workflowPath}:${kind}:${line}:${targetPath}`;
}
function targetState(targetPath: string, kind: RepositoryWorkflowPathReferenceKind, snapshotPaths: ReadonlySet<string>, acceptedPaths: ReadonlySet<string>): RepositoryWorkflowPathTargetState {
...
function parseWorkflowReferences(workflowPath: string, file: RepositoryFileInput, snapshotPaths: ReadonlySet<string>, acceptedPaths: ReadonlySet<string>) {
  const references: RepositoryWorkflowPathReference[] = [];
  let dynamicReferences = 0; let multilineReferences = 0;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (["|", ">", "|-", ">-", "|+", ">+"].includes(rawValue)) { multilineReferences += 1; continue; }
    const targetPath = normalizeWorkflowReference(rawValue);
    if (!targetPath) { dynamicReferences += 1; continue; }
    const line = index + 1;
    references.push({ referenceId: makeReferenceId(workflowPath, kind, targetPath, line), workflowPath, kind, rawReference: stripOptionalQuotes(rawValue), targetPath, targetState: targetState(targetPath, kind, snapshotPaths, acceptedPaths), evidence: { path: workflowPath, line } });
  }
...
function createImpactIndex(references: readonly RepositoryWorkflowPathReference[]): RepositoryWorkflowPathImpact[] {
  ...
  for (const reference of references) {
    const current = byTarget.get(reference.targetPath) ?? { workflows: new Set<string>(), kinds: new Set<RepositoryWorkflowPathReferenceKind>() };
    current.workflows.add(reference.workflowPath); current.kinds.add(reference.kind); byTarget.set(reference.targetPath, current);
  }
  return [...byTarget.entries()].sort(([left], [right]) => compareText(left, right)).map(([targetPath, value]) => ({ targetPath, workflows: [...value.workflows].sort(compareText), referenceKinds: [...value.kinds].sort(compareText) }));
}

export async function createRepositoryWorkflowPathEvidence(snapshot: RepositorySnapshot, graph: SolveGraphDocument, options: RepositoryWorkflowPathEvidenceOptions = {}): Promise<RepositoryWorkflowPathEvidenceAnalysis> {
  const maxReferences = boundedInteger(options.maxReferences, 250, 1, 2_000, "Repository workflow maxReferences");
  const maxWorkflowTextBytes = boundedInteger(options.maxWorkflowTextBytes, 512 * 1024, 1, 5 * 1024 * 1024, "Repository workflow maxWorkflowTextBytes");
  await createSolveGraphQueryIndex(graph);
  if (!sameSource(snapshot, graph)) throw new Error("Repository workflow path evidence source does not match the bounded Solve Graph source.");
  ...
  let missingText = 0; let oversizedText = 0; let dynamicReferences = 0; let multilineReferences = 0; let workflowFilesExamined = 0;
  ...
  for (const workflowPath of [...acceptedPaths].filter(isWorkflowPath).sort(compareText)) {
    ...
    if (textBytes(file.text) > maxWorkflowTextBytes) { oversizedText += 1; continue; }
    const parsed = parseWorkflowReferences(workflowPath, file, snapshotPaths, acceptedPaths);
    dynamicReferences += parsed.dynamicReferences; multilineReferences += parsed.multilineReferences; allReferences.push(...parsed.references);
  }
  allReferences.sort((left, right) => compareText(left.workflowPath, right.workflowPath) || left.evidence.line - right.evidence.line || compareText(left.kind, right.kind) || compareText(left.targetPath, right.targetPath));
  ...
  return {
    ...
    execution: { networkAccess: false, writeAccess: false, maxReferences, maxWorkflowTextBytes, referencesTruncated: allReferences.length > maxReferences, acceptedFiles: acceptedPaths.size, workflowFilesExamined, graphTruncated: graph.execution.truncated },
  };
}
