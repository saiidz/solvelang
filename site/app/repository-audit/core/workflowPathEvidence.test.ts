...
test("maps explicit workflow working directories and cache dependency files", async () => {
  ...
  const analysis = await createRepositoryWorkflowPathEvidence(
    input,
    await graph([".github/workflows/ci.yml", "site/package-lock.json", "site/app/page.tsx"]),
  );

  assert.deepEqual(
    analysis.references.map((reference) => [reference.kind, reference.targetPath, reference.targetState]),
    [
  ...
  assert.deepEqual(analysis.impacts, [
    {
      ...
      referenceKinds: ["working-directory"],
    },
    {
      targetPath: "site/package-lock.json",
      workflows: [".github/workflows/ci.yml"],
...
test("distinguishes outside-scan and missing paths while skipping dynamic or multiline values", async () => {
  const workflow = [
    ...
    "    steps:",
    "      - with:",
    "          cache-dependency-path: missing/package-lock.json",
    "      - run: echo dynamic",
  ...
  const input = snapshot([
    { path: ".github/workflows/verify.yaml", text: workflow },
    { path: "site/package-lock.json", text: "{}" },
  ]);
  const analysis = await createRepositoryWorkflowPathEvidence(
    input,
    await graph([".github/workflows/verify.yaml"]),
  );
...
test("keeps deterministic bounds and fails closed on partial-source or integrity mismatch", async () => {
  ...
  const partial = await createRepositoryWorkflowPathEvidence(
    input,
    await graph([".github/workflows/verify.yml"], true),
    { maxReferences: 1 },
  );
