...
import type { RepositoryAuditImpactExplanationProduct } from "./core/impactExplanationProduct";

type RepositoryAuditImpactExplanationPanelProps = {
  product: RepositoryAuditImpactExplanationProduct | null;
  className?: string;
};
    ...
    return (
      ...
      >
        ...
        <h2 id="repository-audit-impact-explanation-heading" className="mt-2 text-2xl font-semibold text-slate-950">
          Select a graph node to explain its blast radius
        </h2>
        <p className="mt-3 max-w-4xl leading-7 text-slate-700">
          Use the local graph explorer above to select a visible node. SolveLang will explain bounded dependent paths from the already-built canonical graph without executing repository code or accessing the network.
        </p>
      </section>
    );
  }
    ...
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 break-all text-sm font-semibold text-violet-900">
            Selected: {product.selectedNode.label} · {product.selectedNode.kind}
          </p>
        </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-slate-700">
        ...
        <span className="rounded-full bg-white px-3 py-1.5">max observed depth {explanation.summary.maximumObservedDepth}</span>
      ...
      {explanation.rows.length ? (
        <div className="mt-6 grid gap-4">
          {explanation.rows.map((row) => (
            <article key={`${explanation.graphId}:impact:${row.node.id}`} className="rounded-2xl border border-violet-100 bg-white p-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <code className="break-all text-sm font-semibold text-slate-950">{row.node.label}</code>
                <span className="rounded-full bg-violet-100 px-2.5 py-1 text-xs font-bold text-violet-900">depth {row.depth}</span>
              </div>
              <p className="mt-3 text-sm font-semibold text-slate-800">{row.sentence}</p>
                  ...
                  {row.steps.map((step) => (
                    <li key={`${row.node.id}:${step.edgeId}:${step.depth}`} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-500">
                        ...
                        <span className="rounded-full bg-violet-100 px-2.5 py-1 text-violet-900">{step.edgeKind}</span>
                      </div>
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-violet-200 bg-white p-6 text-sm text-slate-600">
          No dependent nodes were established within the configured graph scope.
        </div>
      )}
      ...
      <p className="mt-5 text-xs leading-5 text-slate-500">
        Analyze only · product schema {product.schema} · explanation schema {explanation.schema} · network access disabled · write access disabled
      </p>
    </section>
  );
}
