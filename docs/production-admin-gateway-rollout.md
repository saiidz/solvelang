...
- production API/customer accounts/Admin CRM remain enabled; record and preserve the current billing flag, whether enabled or disabled.
...
1. captures the live API/customer/CRM/billing baseline using the read-only role and verifies health reports the same billing flag;
...
7. re-checks Admin CRM and proves the billing parameter and health flag still match the pre-deployment value;
