...
- `main` at this sync: `4448c06413af714755f67ffa3bf12342d10bba75`.
...
These PRs are build-only preparation and remain intentionally unmerged. Because `main` advanced after their last refresh, replay each on the then-current `main` before any approved merge and rerun exact-head CI.

- **#161** `fix(prod): preserve Admin CRM through auth rollbacks` — gate: `APPROVE PR #161 MERGE`.
- **#164** `feat(priority): add validation-only production preflight` — gate: `APPROVE PR #164 MERGE`.
- **#169** `ops(priority): prepare dormant production queue foundation rollout` — gate: `APPROVE PR #169 MERGE`.
...
- **#188** — deterministic repository-local Python import relationships and multi-language graph composition. Hosted CI/Rust were green on its last exact head; Trusted Mac validation remains pending/queued. Reconcile it on current `main` before merge and do not bypass that Mac gate.
...
Runner inventory is not exposed by the current connector. Infer only from GitHub Actions run/job metadata. Current exact-head Mac jobs for the trusted branches above remain queued/pending; do not interrupt them, rewrite runner registration, weaken branch trust, or route them to Windows merely to bypass the queue.
...
Server Audit is a real read-only implementation on `main`, not a roadmap-only stub. Current merged code includes strict snapshot parsing, deterministic findings/report generation, browser-local analysis, constrained collector contracts, temporal consistency analysis (#198), and temporal report composition (#199).
...
- **#201** — adds bounded read-only consistency checks for conflicting duplicate package, service, filesystem, and web-root inventory evidence. Reconcile its head on current `main` before merge and require exact-head hosted CI.
