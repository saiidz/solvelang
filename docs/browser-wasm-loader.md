...
The Node-only prebuild check verifies the exact file set, checksums, current artifact policy, import/export surface and byte limits. The authoritative Linux WASM CI additionally requires the pinned payload files to match a fresh audited build of current source. Local macOS builds qualify reproducibility within that host and browser behavior, but currently emit different bytes from Linux; cross-host byte reproducibility is not claimed. Next's bundler root includes the repository only to resolve the shared adapter; static export remains enabled and the production site build needs no Rust toolchain.
