...
The experimental `solvelsp` stdio binary currently supports full-document open diagnostics plus parser-backed document symbols, top-level go-to-definition, hover, and same-name source highlights for documents opened in the current session. Incremental changes, workspace access, and execution are intentionally not implemented.
