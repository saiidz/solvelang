...
The experimental `solvelsp` stdio binary currently supports full-document open diagnostics plus parser-backed document symbols, top-level go-to-definition, hover, same-name source highlights, top-level-symbol completion, parser-validated keyword/identifier/number/operator semantic tokens, and parser-gated full-document formatting through the canonical formatter for documents opened in the current session. Incremental changes, workspace access, and execution are intentionally not implemented.
