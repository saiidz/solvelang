...
export function createGmailSupportProvider({ credentialResolver, fetchImpl = fetch, now = Date.now }) {
  ...
  async function refreshToken(secretArn, secret) {
    ...
    const form = new URLSearchParams({ grant_type: "refresh_token", refresh_token: secret.refreshToken, client_id: secret.clientId, client_secret: secret.clientSecret });
    const body = await fetchJson(fetchImpl, "https://oauth2.googleapis.com/token", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: form.toString() }, "Gmail OAuth");
  ...
  async function token(secretArn) {
    ...
    if (typeof secret.accessToken === "string" && secret.accessToken.length >= 20 && (!Number.isFinite(expiresAt) || expiresAt > now() + 60_000)) return secret.accessToken;
    return refreshToken(secretArn, secret);
  }

  async function request(secretArn, path, options = {}) {
    const accessToken = await token(secretArn);
    return fetchJson(fetchImpl, `https://gmail.googleapis.com/gmail/v1${path}`, { ...options, headers: { authorization: `Bearer ${accessToken}`, "content-type": "application/json", ...(options.headers ?? {}) } }, "Gmail");
  }

  return {
    async getProfile({ credentialSecretArn, mailbox }) { return request(credentialSecretArn, `/users/${encodeURIComponent(mailbox)}/profile`, { method: "GET" }); },
    async listUnread({ credentialSecretArn, mailbox, limit = 5 }) {
      const query = new URLSearchParams({ q: `is:unread newer_than:30d -from:me to:${cleanAddress(mailbox)}`, maxResults: String(Math.min(Math.max(limit, 1), 20)) });
    ...
    async getMessage({ credentialSecretArn, mailbox, id }) {
      ...
      return { id: body.id, threadId: body.threadId, rfcMessageId: header(body.payload, "Message-ID"), from: cleanAddress(header(body.payload, "From")), to: cleanAddress(header(body.payload, "To") || mailbox), subject: header(body.payload, "Subject") || "Support request", text: plainText(body.payload), receivedAt: body.internalDate ? new Date(Number(body.internalDate)).toISOString() : undefined };
    },
    async markRead({ credentialSecretArn, mailbox, id }) {
      const body = await request(credentialSecretArn, `/users/${encodeURIComponent(mailbox)}/messages/${encodeURIComponent(id)}/modify`, { method: "POST", body: JSON.stringify({ removeLabelIds: ["UNREAD"] }) });
  ...
  return {
    async createIssue({ credentialSecretArn, teamId, title, description }) {
      ...
      const body = await fetchJson(fetchImpl, "https://api.linear.app/graphql", {
        ...
        body: JSON.stringify({ query: "mutation SupportIssue($input: IssueCreateInput!) { issueCreate(input: $input) { success issue { id url identifier } } }", variables: { input: { teamId, title: String(title).slice(0, 240), description: String(description).slice(0, 8_000) } } }),
      }, "Linear");
