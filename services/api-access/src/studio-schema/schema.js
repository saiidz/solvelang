...
// Source SHA-256: 8130c3ec43712ba57d7334018d0af422b36891fca5589a7bf62ce160b5713610
import { z } from "zod";
import { NODE_TYPES } from "./types.js";
const MAX_TEXT = 100_000;
const text = z.string().max(MAX_TEXT);
const safeRecordKey = (key) => !["__proto__", "prototype", "constructor"].includes(key);
const stringMap = z.record(z.string(), text).superRefine((value, context) => {
    for (const key of Object.keys(value))
        if (!safeRecordKey(key))
            context.addIssue({ code: "custom", path: [key], message: "Unsafe object key is not allowed." });
});
...
export const WorkflowDocumentSchema = z.object({
    ...
    const unique = (items, path) => {
        ...
        items.forEach((item, index) => {
            if (seen.has(item.id))
                context.addIssue({ code: "custom", path: [path, index, "id"], message: `Duplicate ${path.slice(0, -1)} ID ${item.id}.` });
            seen.add(item.id);
        });
    };
    unique(document.nodes, "nodes");
    unique(document.edges, "edges");
    unique(document.scenarios, "scenarios");
    unique(document.policies, "policies");
    ...
    document.edges.forEach((edge, index) => {
        if (!nodes.has(edge.source))
            context.addIssue({ code: "custom", path: ["edges", index, "source"], message: `Unknown source node ${edge.source}.` });
        if (!nodes.has(edge.target))
            context.addIssue({ code: "custom", path: ["edges", index, "target"], message: `Unknown target node ${edge.target}.` });
    });
    document.nodes.forEach((node, index) => node.policyRefs.forEach((policyId, policyIndex) => {
        if (!policies.has(policyId))
            context.addIssue({ code: "custom", path: ["nodes", index, "policyRefs", policyIndex], message: `Unknown policy ${policyId}.` });
    }));
    document.scenarios.forEach((scenario, index) => {
        const trigger = nodes.get(scenario.startingTrigger);
        if (!trigger || trigger.type !== "trigger")
            context.addIssue({ code: "custom", path: ["scenarios", index, "startingTrigger"], message: `Starting trigger ${scenario.startingTrigger || "(missing)"} must reference a trigger node.` });
        if (scenario.expectedTerminalState && nodes.get(scenario.expectedTerminalState)?.type !== "terminal")
            context.addIssue({ code: "custom", path: ["scenarios", index, "expectedTerminalState"], message: `Expected terminal ${scenario.expectedTerminalState} must reference a terminal node.` });
        Object.keys(scenario.decisionOutcomes).forEach((nodeId) => {
            if (nodes.get(nodeId)?.type !== "decision")
                context.addIssue({ code: "custom", path: ["scenarios", index, "decisionOutcomes", nodeId], message: `Decision outcome key ${nodeId} must reference a decision node.` });
        });
        scenario.expectedHumanReviewPoints.forEach((nodeId, reviewIndex) => {
            const node = nodes.get(nodeId);
            if (!node || !(node.humanRequired || node.type === "human_review" || node.type === "approval"))
                context.addIssue({ code: "custom", path: ["scenarios", index, "expectedHumanReviewPoints", reviewIndex], message: `Expected review ${nodeId} must reference a human-review node.` });
        });
        scenario.expectedOutputs.forEach((output, outputIndex) => {
            if (!outputs.has(output))
                context.addIssue({ code: "custom", path: ["scenarios", index, "expectedOutputs", outputIndex], message: `Expected output ${output} is not produced by any node.` });
        });
    });
});
...
function containsUnsafeObjectKey(input, seen = new Set()) {
    if (!input || typeof input !== "object" || seen.has(input))
        return false;
    seen.add(input);
    if (Object.keys(input).some((key) => !safeRecordKey(key)))
        return true;
    return Object.values(input).some((value) => containsUnsafeObjectKey(value, seen));
}
export function parseWorkflowDocument(input) {
    if (containsUnsafeObjectKey(input))
        return { ok: false, error: "document: Unsafe object key is not allowed." };
