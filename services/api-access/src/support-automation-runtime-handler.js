...
export function parseSupportAutomationRuntimeEnvironment(environment = process.env) {
  ...
  return { enabled, activationEnabled, runtimeMode, siteOrigin: required(environment, "SITE_ORIGIN"), supportAutomationTable: enabled ? required(environment, "API_SUPPORT_AUTOMATION_TABLE") : undefined, customerAuthTable: enabled ? required(environment, "API_CUSTOMER_AUTH_TABLE") : undefined, customerAuthPepper: enabled && runtimeMode === "api" ? required(environment, "API_CUSTOMER_AUTH_PEPPER", 32) : undefined, approvedMailHosts: enabled ? approvedMailHosts(environment.API_SUPPORT_AUTOMATION_MAIL_HOSTS) : [], mailSendEnabled: enabled && environment.API_SUPPORT_AUTOMATION_MAIL_SEND_ENABLED === "true" };
}
