...
test("returns a sanitized dashboard without exposing fingerprints or Stripe identifiers", async () => {
  const store = {
    getAccount: async () => ({
      accountId: "acct_1",
  ...
  assert.equal(dashboard.usage.limit, 10_000);
...
test("shows an unsubscribed customer shell before Checkout", async () => {
  const service = createCustomerAccountService({
    store: { getAccount: async () => undefined, listKeys: async () => [] },
    apiAccessService: { issueApiKey() {}, revokeApiKey() {} },
    now: () => fixedNow,
