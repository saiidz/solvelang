...
test("priority deployment is protected, manual, main-only, and test-only", async () => {
  ...
  assert.doesNotMatch(source, /PriorityQueueMode="live"/);
});

test("canary deployment proves all four weighted worker lanes", async () => {
  const source = await workflow();
