...
test("issues and verifies an entitlement", () => {
  const token = issueEntitlement({ version: 1, scanId: "scan-1", sessionId: "pi_test_1", exp: 2000 }, secret);
  const claims = verifyEntitlement(token, secret, 1000);
  assert.equal(claims.scanId, "scan-1");
  assert.equal(claims.sessionId, "pi_test_1");
});
...
test("rejects expired tokens", () => {
  const token = issueEntitlement({ version: 1, scanId: "scan-1", sessionId: "pi_test_1", exp: 1000 }, secret);
