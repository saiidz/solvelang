...
test("Turnstile verification rejects unsuccessful, hostname-mismatched, action-mismatched, and malformed responses", async () => {
  ...
  assert.equal(await hostnameMismatch.verify({ token: "invalid", remoteIp: "127.0.0.1" }), false);
  ...
  assert.equal(await actionMismatch.verify({ token: "invalid", remoteIp: "127.0.0.1" }), false);
  ...
  assert.equal(await malformed.verify({ token: "invalid", remoteIp: "127.0.0.1" }), false);
});
...
test("Turnstile verification fails closed for malformed JSON, timeouts, and network failures", async () => {
  ...
  assert.equal(await malformedJson.verify({ token: "valid", remoteIp: "127.0.0.1" }), false);
  ...
  await assert.rejects(timeout.verify({ token: "valid", remoteIp: "127.0.0.1" }));
  ...
  await assert.rejects(networkFailure.verify({ token: "valid", remoteIp: "127.0.0.1" }));
});
