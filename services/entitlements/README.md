...
- The dispatcher reads `DURABLE_CONFIRMATION_PROVIDER` and routes each pending outbox record fail closed. `aws-ses-sqs` sends to the encrypted FIFO SQS queue and marks the record dispatched only after SQS accepts it, preserving safe replay after queue or update ambiguity. `test-sink` makes no SQS or SES call, requires no SES sender, retains only the opaque idempotency identifier, and then marks the record dispatched. `disabled` sends nothing and leaves the record pending.
...
- `disabled`, `test-sink`, and `aws-ses-sqs` are the only durable confirmation providers. Test mode can use the privacy-safe dispatcher sink; production rejects it and requires the encrypted FIFO SQS queue, DLQ, SES worker, and a verified sender before checkout can be enabled.
